...
def initialize_aggregate_df(metric_cols, types, model, input):
    ...
    for deployment_mechanism in types:
...
def analyze_data_significant_difference(df, significance_level, metrics, model, model_with_underscores,
    input, input_with_underscores, analyzed_results_path, include_insignificant_output=False):
    ...
    types = df["deployment-mechanism"].unique()
    ...
    aggregate_df = initialize_aggregate_df(metrics, types, model, input)
    ...
    for deployment_mechanism_x, deployment_mechanism_y in combinations(types, 2):
        ...
        for metric in metrics:
            ...
            if statistically_significant:
                ...
                print(f"Statistically significant difference between {deployment_mechanism_x} and {deployment_mechanism_y} for {metric}")
            else:
                print(f"No statistically significant difference between {deployment_mechanism_x} and {deployment_mechanism_y} for {metric}")
                if not include_insignificant_output:
                    continue

            print(f"Mean difference: {mean_diff:.2f} ± {ci_half_width:.2f} with confidence level {(1 - significance_level) * 100.0}%")
            print(f"{deployment_mechanism_x} average: {x_mean:.2f} (95% CI: {x_ci[0]:.2f} to {x_ci[1]:.2f})")
            print(f"{deployment_mechanism_y} average: {y_mean:.2f} (95% CI: {y_ci[0]:.2f} to {y_ci[1]:.2f})")
            ...
            if x_mean < y_mean:
                ...
                print(f"{deployment_mechanism_y} is {ratio_min:.2f} to {ratio_max:.2f} times larger than {deployment_mechanism_x} for {metric}\n")
            else:
                ratio = x_mean / y_mean
                ratio_ci = ci_half_width / y_mean
                ratio_min = ratio - ratio_ci
                ratio_max = ratio + ratio_ci
                print(f"{deployment_mechanism_x} is {ratio_min:.2f} to {ratio_max:.2f} times larger than {deployment_mechanism_y} for {metric}\n")
...
def parse_csv_rows(results_filename, deployment_mechanisms, metrics, include_docker_overhead=True, is_perf_file=True):
    ...
    if include_docker_overhead:
        ...
        # TODO: replace deployment_mechanism with deployment_mechanism
...
def plot_metrics_bar_chart(aggregate_df, metrics):
...
def main():
    ...
    parser.add_argument("--exclude-docker-overhead", action="store_true", default=False, help="Exclude Docker overhead in the analysis.")
    parser.add_argument("--include-insignificant-output", action="store_true", default=False, help="Include statistical comparisons when they are not statistically significant.")
    # TODO: replace comma-separateds with lists and nargs
    ...
    perf_filename = f"{model_with_underscores}_{input_with_underscores}_perf_results.csv"
    time_filename = f"{model_with_underscores}_{input_with_underscores}_time_results.csv"
    ...
    #perf_df = parse_csv_rows(perf_path, deployment_mechanisms, metrics, model, input, not args.exclude_docker_overhead)
    ...
    #plot_metrics_bar_chart(perf_aggregate_df, metrics
    plot_metrics_bar_chart(time_aggregate_df, time_metrics)
    
    #perf_aggregate_csv_filepath = os.path.join(experiments_set_path, PERF_AGGREGATE_CSV_FILENAME)
    time_aggregate_csv_path = os.path.join(analyzed_results_path, TIME_AGGREGATE_CSV_FILENAME)
    create_or_update_aggregate_csv(time_aggregate_df, time_aggregate_csv_path)
