...
function main() {
    ...
    suite_directory="$suite_path/$suite_name"
...
function setup_cadvisor() {
    chmod u+x $suite_directory/cadvisor/cadvisor
    sudo apt-get install libpfm4 linux-perf
}

function setup_prometheus() {
    chmod u+x $suite_directory/prometheus/prometheus
}

function setup_python() {
    sudo apt install -y python3 python3-venv python3-pip cgroup-tools
    python3 -m venv "$suite_directory/python/myenv" 
    source "$suite_directory/python/myenv/bin/activate" && pip install -r requirements.txt
}

function aot_compile_wasm_non_mac() {
    # AoT compile the WebAssembly code (if not on Mac)
    wasmedge compile "$suite_directory/wasm/interpreted.wasm" "$suite_directory/wasm/aot.wasm"
}

function aot_compile_wasm_mac() {
    # AoT compile the WebAssembly code (if on Mac)
    wasmedge compile "$suite_directory/wasm/interpreted.wasm" "$suite_directory/wasm/aot.so"
}
